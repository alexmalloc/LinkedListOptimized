...
public class LinkedListFork<T> implements List<T> {
    ...
    Node first;
    Node last;
    Node[] indexArr = (Node[]) Array.newInstance( Node.class, 100);
    ...
    public Node findNode(int i) {
         if(i < size()) {
                if(i < indexArr.length * 100) {
                    ...
                    Node current = indexArr[indexArr.length-1];
                    for (int r = indexArr.length-1; r < size(); r++) {
                        if (r == i) {
                            if (current != null) {
